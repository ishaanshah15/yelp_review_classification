...
def load_data(train_file,test_file,batch_sz,window_sz):
    ...
    word2id,counts = {},defaultdict(int)
    ps = PorterStemmer()
    count_vocab(counts,train_df['reviews'].tolist(),test_df['reviews'].tolist(),ps)
    tr_inputs,tr_lengths = preprocess(train_df['reviews'].tolist(),word2id,counts,window_sz,ps)
    te_inputs,te_lengths = preprocess(test_df['reviews'].tolist(),word2id,counts,window_sz,ps)
...
def count_vocab(counts,train_reviews,test_reviews,ps):
...
def preprocess(reviews,word2id,counts,window_sz,ps):
    inputs = []
    lengths = []
    for review in reviews:
        ...
        for w in words:
            stem = ps.stem(w)
            if counts[stem] < 20: # UNK threshold
                res.append(add_word("UNK",word2id))
            else:
                res.append(add_word(stem,word2id))
